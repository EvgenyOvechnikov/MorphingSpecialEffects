...
int
MyLoadObjFile(char* name, std::vector <struct Vertex>& OutVertices, std::vector <struct Normal>& OutNormals, std::vector <struct TextureCoord>& OutTextureCoords)
{
	OutVertices.clear();
	OutNormals.clear();
	...
	while (true)
	{
		...
		char* tempContext;
		cmd = strtok_s(line, OBJDELIMS, &tempContext);
		...
		if (strcmp(cmd, "v") == 0)
		{
			str = strtok_s(NULL, OBJDELIMS, &tempContext);
			sv.x = atof(str);
		...
		if (strcmp(cmd, "f") == 0)
		{
			...
			int vtx = 0;
			char* str;
			while ((str = strtok_s(NULL, OBJDELIMS, &tempContext)) != NULL)
			{
				int v, n, t;
